...
import { fetchUserAttendanceRequests } from "@/redux/slices/attendance-request/user/userAttendanceRequestSlice";
import { SearchInput } from "@/components/ui/searchbox";
import { Button } from "@/components/ui/button";
import { ChevronLeft, ChevronRight } from "lucide-react";
import { IoInformationCircleOutline } from "react-icons/io5";

export default function MyCorrectionRequestsPage() {
  const dispatch = useAppDispatch();
  const {
    requests,
    loading: status,
    error,
  } = useAppSelector((state) => state.userAttendanceRequest);
  ...
  useEffect(() => {
    dispatch(fetchUserAttendanceRequests());
  }, [dispatch]);
  ...
  const filteredData = useMemo(() => {
    ...
    return data;
  }, [requests, globalFilter, statusFilter, typeFilter]);

  const handleView = (row: CorrectionRequestRow) => {
    console.log("View Request:", row);
  };

  const columns = useMemo(() => getColumns({ onView: handleView }), []);
