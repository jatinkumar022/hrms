...
import { getColumns } from "./requestsData";
...
export default function WfhRequestsTable() {
  ...
  const { users: allUsers, status: usersStatus } = useAppSelector(
  ...
  const filteredData = useMemo(() => {
    let data: WfhRequest[] = allWfhRequests || [];
    if (globalFilter) {
      data = data.filter(
        (req) =>
          req.user.username
          ...
          req.reason.toLowerCase().includes(globalFilter.toLowerCase())
      );
    }
    if (userFilter && userFilter !== "all") {
      data = data.filter((req) => req.user._id === userFilter);
    }
    if (yearFilter && yearFilter !== "all") {
      data = data.filter((req) => req.startDate.startsWith(yearFilter));
    }
    if (statusFilter && statusFilter !== "all") {
      data = data.filter((req) => req.status === statusFilter);
    }
    if (typeFilter && typeFilter !== "all") {
      data = data.filter((req) => req.dayType === typeFilter);
    }
    return data;
  }, [
    allWfhRequests,
  ...
  const handleApprove = (id: string) => {
    dispatch(updateWfhRequestStatus({ wfhId: id, status: "approved" }));
  };

  const handleReject = (id: string) => {
    dispatch(updateWfhRequestStatus({ wfhId: id, status: "rejected" }));
  };

  const handleView = (row: WfhRequest) => {
    console.log("Viewing WFH Request:", row);
    // TODO: Implement a drawer or modal to show WFH request details
  };
  ...
  return (
    <>
      <FullPageLoader
        show={status === "loading" || usersStatus === "loading"}
      />
